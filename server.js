...
const CATEGORY_RULES = [
  ...
  { category: 'Billing Issue', patterns: [
    [/(hidden|undisclosed|surprise|unexpected|added|extra)\s+(fee|fees|charge|charges|cost|costs)/i, 6],
    [/(not|never)\s+(told|informed|disclosed|advised|notified)\s+(about|of|that|us|me)?/i, 4],
    [/(disconnect|termination|cancellation|early\s+termination)\s+fee/i, 5],
    [/carrier\s+recovery\s+fund/i, 5],
    [/(assured|promised|told|quoted)\s+.{0,40}\$\d/i, 5],
    [/(shocked|surprised)\s+(to\s+see|by)\s+(the\s+)?(charge|bill|cost|fee)/i, 6],
    [/didn'?t\s+(know|realize)\s+.{0,30}(charge|fee|cost|bill)/i, 5],
    [/unauthorized\s+(charge|charges|billing|payment)/i, 6],
    ...
    [/lack\s+of\s+transparency\s+(regarding|about|with|on)\s+(the\s+)?(billing|charges|minutes\s+billed|call\s+volume)/i, 5],
    [/(unclear|unpredictable|opaque)\s+(billing|invoicing|charges)/i, 5],
...
- "improper billing", "improperly billed", "incorrect billing", "wrong billing", "erroneous billing", "unauthorized charge", "unauthorized charges", "unauthorized billing", "billing error", "billing dispute", "billing issue", "billing discrepancy", "billing concern (dispute-flavored)", "billing problem", "billed incorrectly", "billed wrong", "billed twice", "double-charged", "duplicate charge", "improperly charged", "wrongly charged", "incorrectly charged", "credit card dispute", "cc dispute", "chargeback", "filing a dispute", "disputing charges", "disputing the bill/invoice/charges", "lack of transparency in billing", "lack of transparency regarding minutes billed", "lack of transparency regarding call volume/charges", "unclear billing", "unpredictable billing", "hidden fees", "undisclosed fees", "surprise fees", "unexpected costs", "added costs I wasn't told about", "not told about the charges/fees", "never informed of the fees", "shocked to see the added costs", "disconnect fee (undisclosed)", "carrier recovery fund (undisclosed)", "early termination fee (undisclosed)", "assured $X but charged Y", "quoted one price, billed another", "charged despite downgrade", "charged despite cancellation", "still billed after cancellation", "still charged after I cancelled", "billed after cancellation", "charged after we cancelled", "charged the wrong plan", "charged the wrong amount", "disputed charge", "disputed invoice", "wrong amount on bill/invoice", "invoice is wrong", "refund for improper charge", "refund owed", "should not have been charged" → "Billing Issue"
...
- ⚠️ BILLING DOMINATES QUALITY (READ CAREFULLY): When a ticket contains BOTH a substantive billing complaint AND a service-quality complaint, "Billing Issue" wins — do NOT default to "Quality". A substantive billing complaint is any specific billing/invoicing grievance: named fees the customer didn't know about (disconnect fee, carrier recovery fund, hidden fees, undisclosed fees, added costs), promised prices that weren't honored ("assured $X but charged Y"), disputed charges, unauthorized charges, invoice errors, charges after cancellation, refund requests for specific amounts, or "shocked to see" language about the bill. "Quality" is reserved for tickets DOMINANTLY about service delivery (missed calls, wrong messages, rude receptionists) with billing being incidental or unmentioned. If you find yourself writing reasoning like "this is a billing dispute AND a quality issue" — pick Billing Issue.
...
- Undisclosed / surprise fees = Billing Issue: When the customer says they were "not told about", "not informed of", "not disclosed", "shocked to see", or "assured a lower price than they were charged" — that is "Billing Issue" (the customer's complaint is that fees weren't communicated / promises weren't kept). Named fee types that commonly show up here: disconnect fee, cancellation fee, early termination fee, carrier recovery fund, activation fee, minute overage, admin fee.
...
- Billing Issue vs. Quality — PRIMARY REASON wins: When the customer explicitly says the billing problem was the biggest / primary / main / #1 reason they cancelled, use "Billing Issue" even if the ticket ALSO mentions service quality complaints. Look for phrases like "biggest reason", "primary reason", "the reason we cancelled was", "what pushed us out was" — the customer is telling you which cause dominated. Do not lump billing complaints into "Quality" just because a quality grievance was also mentioned; the customer's stated primary driver is authoritative.
- Billing transparency = Billing Issue: When the customer complains about "lack of transparency" regarding billing, minutes billed, call volume charged, or unclear/unpredictable invoicing — that is "Billing Issue". This is a billing/invoicing complaint, not a service quality complaint.
